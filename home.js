...
function renderStudentTable(bodyId, students, n) {
  ...
  if (head) {
    head.innerHTML = '<tr><th style="width:40px">#</th><th>Name</th><th>Stream</th><th>Batch</th>' +
      '<th class="text-center">Tests Taken / Total Test</th>' +
      '<th class="text-center">Avg Score (Avg %)</th>' +
  ...
  body.innerHTML = rows.map((s, i) => `
    <tr>
      ...
      <td class="text-center"><strong>${s.avgUserScore || 0}</strong> <span class="status-badge ${scoreBadge(s.avg)}">(${s.avg}%)</span></td>
...
function tableExportData(key) {
  ...
  if (key === 'topper' || key === 'bottom') {
    ...
    return {
      ...
      headers: ['#', 'Name', 'Reg No', 'Stream', 'Batch', 'Tests Taken / Total Test', 'Avg Score (Avg %)', ...subs.map(s => SUBJ_LABELS[s])],
